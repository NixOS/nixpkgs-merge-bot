{ inputs, ... }:
{
  flake.nixosConfigurations.nixpkgs-merge-bot-staging = inputs.nixpkgs.lib.nixosSystem {
    modules = [
      (
        { config, lib, ... }:
        {
          nixpkgs.hostPlatform = "x86_64-linux";
          users.users.root.openssh.authorizedKeys.keys = [
            "ssh-rsa " # lassulus
            "sk-ssh-ed25519@openssh.com AAAAGnNrLXNzaC1lZDI1NTE5QG9wZW5zc2guY29tAAAAIEVSsc5mlP8aWiUVwWWM3gKlB5LHVpmKSifnDyox/BnVAAAABHNzaDo= yubikey1" # Mic92
            "ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBCsjXKHCkpQT4LhWIdT0vDM/E/3tw/4KHTQcdJhyqPSH0FnwC8mfP2N9oHYFa2isw538kArd5ZMo5DD1ujL5dLk= ssh@secretive.Joerg’s-Laptop.local" # Mic92
            "sk-ssh-ed25519@openssh.com AAAAGnNrLXNzaC1lZDI1NTE5QG9wZW5zc2guY29tAAAAIP4MIZG/hZR3Ib7faGDyK67Tk53Q1P7pE5cFIWwEFbrtAAAABHNzaDo=" # Fritz Otlinghaus, Scriptkiddi
            "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPqY9E43rQl3MHlE9L5cAqbdtePPNZADG4LGuQyzkgh6" # _dominic
            "ssh-rsa " # Wolfgang Walther
          ];
          boot.loader.grub.devices = lib.mkForce [ "/dev/sda" ];

          sops.secrets."staging/webhook_secret" = { };
          sops.secrets."staging/github_app_key" = { };
          services.nixpkgs-merge-bot = {
            enable = true;
            hostname = "nixpkgs-merge-bot-staging.nixos.org";
            github-app-login = "nixpkgs-merge";
            github-app-id = 410668;
            webhook-secret-file = config.sops.secrets."staging/webhook_secret".path;
            github-app-private-key-file = config.sops.secrets."staging/github_app_key".path;
          };

          systemd.services.nixpkgs-merge-bot.environment = {
            LOGLEVEL = "DEBUG";
          };

          system.stateVersion = "23.11";

          security.acme.acceptTerms = true;
          security.acme.defaults.email = "nix@lassul.us";

          system.autoUpgrade = {
            enable = false;
            allowReboot = true;
            flake = "github:nixos/nixpkgs-merge-bot#nixpkgs-merge-bot-staging";
            dates = "*:0/10:00";
            flags = [ "--refresh" ];
          };
        }
      )
      ./modules/nixpkgs-merge-bot.nix
      ./modules/sops.nix
      ./disk-config.nix

      inputs.sops-nix.nixosModules.sops
      inputs.disko.nixosModules.disko

      inputs.srvos.nixosModules.hardware-hetzner-cloud
      inputs.srvos.nixosModules.server
      inputs.srvos.nixosModules.mixins-nginx
    ];
  };
}
